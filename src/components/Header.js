...
const Header = () => {
  ...
  const getBrandColor = (brandSlug) => {
    const brandColors = {
      'afrismocks': '#1E40AF', // Blue
      'la-veeda': '#1b4332',   // La Veeda green
      'ogribusiness': '#059669' // OgriBusiness green
    };
    return brandColors[brandSlug] || '#000000';
  };
                  ...
                  {Array.isArray(brands) && brands.map((brand) => (
                    ...
                    >
                      <div 
                        ...
                        style={{ backgroundColor: getBrandColor(brand.slug) }}
                      ></div>
              ...
              <div className="space-y-2">
                ...
                {Array.isArray(brands) && brands.map((brand) => (
                  <Link
                    key={brand._id || brand.id}
                    to={`/brand/${brand.slug}`}
                    className="flex items-center space-x-3 pl-4 text-sm text-gray-600 hover:text-golden-600 transition-colors"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    <div 
