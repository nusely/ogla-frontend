...
const Requests = () => {
  ...
  const handleStatusUpdate = async (requestId, newStatus) => {
    ...
    try {
      ...
      if (response.data?.emailSent) {
        alert(`Request ${newStatus} successfully! Email notification sent.`);
      } else {
  ...
  const handleDownloadPdf = async (request) => {
    // Try different possible field names for request number
    const requestNumber = request.request_number || request.requestNumber || request.id;
    if (!requestNumber) {
